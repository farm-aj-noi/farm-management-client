...
import Pdf from "./report_paper/3_Entrail";
import Excel from "./report_excel/3_Entrail";
...
export const QUERY = gql`
  query QUERY($startdate: String, $enddate: String) {
    Reportentrail(startdate: $startdate, enddate: $enddate) {
      imslaughter {
        numkun
      }
      offal
      ...
      >
        <>
          ...
          >
            ...
            <DivFromDown>
              <div style={{ margin: "auto", minWidth: "100%", float: "right", marginBottom:"15px" }}>
                {data && data.Reportentrail.length > 0 ? (
                  <div>
                  <Pdf prod={data.Reportentrail} /> <Excel prod={data.Reportentrail} />{" "}
                </div>
                ) : (
                  ""
                )}
                ...
                >
                  ...
                  <tbody>
                    {data && data.Reportentrail.length > 0 ?
                      data.Reportentrail.map((prod) => (
                        <tr key={prod.id} style={{ textAlign: "center" }}>
                          <td>{prod.imslaughter.numkun}</td>
                          <td>{prod.offal}</td>
                          <td>{prod.toe}</td>
                          <td>{prod.head}</td>
                          <td>{prod.skin}</td>
