...
export const QUERY = gql`
  query QUERY($startdate: String, $enddate: String) {
    ReportgetTreat(startdate: $startdate, enddate: $enddate) {
      numcow
      numkun
      pun
      numfarmer
            ...
            <DivFromDown>
              ...
              >
                {data && data.ReportgetTreat.length > 0 ? (
                  <div>
                    <Pdf prod={data.ReportgetTreat} />{" "}
                    <Excel prod={data.ReportgetTreat} />{" "}
                  </div>
                ) : (
                  ""
                )}
                    ...
                    {data && data.ReportgetTreat.length > 0 ? (
                      data.ReportgetTreat.map((prod) => (
                        <tr key={prod.id} style={{ textAlign: "center" }}>
                          ...
                          <td>
                            {prod.treats.map((treat) => (
                              <div>{treat.dise} <br/></div> 
                            ))}
                          </td>
                          <td>
                          {prod.treats.map((treat) => (
